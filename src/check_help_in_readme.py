...
# We could not write this in Powershell to save our lives. Any volunteers are more than
# welcome!
...
_THIS_DIR = pathlib.Path(os.path.realpath(__file__)).parent
...
def parse_readme(lines: List[str]) -> Tuple[List[Block], List[str]]:
    ...
    while i < len(lines):
        ...
        if mtch:
            ...
            if end_index == -1:
                return [], [
                    "Could not find the end marker {!r}".format(help_ends)
                ]
...
def capture_output_lines(command: str) -> List[str]:
    ...
    if command_parts[0] in ["python", "python3"]:
        ...
        command_parts[0] = str(REPO_ROOT / command_parts[0])
...
def output_lines_to_code_block(output_lines: List[str]) -> List[str]:
    """Translate the output of a help command to a Markdown code block."""
    result = ["```"] + output_lines + ["```"]
